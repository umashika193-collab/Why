...
* **市場・開示データに基づく動的選定**:
  全10セクターの候補プール（AIインフラ、防衛、バイオ・GLP-1、先端半導体、オルタナティブ金融、重要鉱物、宇宙防衛、クリーンエネルギー等）から、直近四半期で最も資本が集約している**上位5大セクターを動的に選出・ランク付け**。
* **3層マネーフロー・フィルターによる複合集計**:
  短期的な市場ボラティリティによるノイズを抑制し、中長期の構造的資本動向を捉えるため、**「① SEC 13F / 企業確定CAPEX（基準資本）」×「② 60日平滑化 ETF資金流向（中期モメンタム）」×「③ SEC Form 8-K/13G 重要開示」** を統合した総合資本流入額（$B/QTR）で順位を算出。
* **完全連動データパネル**:
